...
- implicit BDF/IDA-class stiff integration;
...
5. Integrate `dx/dt = g(t, x, y)` with adaptive local-error control.
6. Shorten a step at every declared time event.
7. Return the trace, accepted/rejected step counts and graph summary.
...
Before adding a stiff or sparse backend, its adapter must keep these tests and
add solver-specific benchmark suites, tolerance-convergence studies and
cross-validation against a trusted external implementation. The UI may then
describe the capability that passed—not the capability that is merely planned.
