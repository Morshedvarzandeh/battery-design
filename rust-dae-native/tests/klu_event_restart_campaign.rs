...
use battery_design_dae_native::{
    ...
    IdaKluSettings, IdaSolveResult, MAX_EVENT_RESTARTS, NATIVE_IDA_BACKEND_CONTRACT,
    NATIVE_IDA_BACKEND_ID, NATIVE_IDA_KLU_BACKEND_CONTRACT, NATIVE_IDA_KLU_BACKEND_ID,
    NATIVE_IDA_KLU_RESULT_CONTRACT, NATIVE_IDA_RESULT_CONTRACT, PINNED_KLU_VERSION,
...
const CAMPAIGN_JACOBIAN_EVALUATIONS: u64 = 10_000;
...
fn klu_settings(
    ...
    let nonzeros_u64 = u64::try_from(nonzeros).expect("fixture nonzeros fit u64");
    let max_known_csc_bytes = IdaKluSettings::known_csc_bytes(dimension, nonzeros)
        .expect("fixture CSC storage is representable");
    let max_jacobian_entry_work = CAMPAIGN_JACOBIAN_EVALUATIONS
        .checked_mul(nonzeros_u64)
        .expect("fixture Jacobian work is representable");
    let max_result_values = output_times_s
        .len()
        .checked_mul(system.outputs().len())
        .expect("fixture result shape is representable");
    IdaKluSettings {
        initial_time_s,
        output_times_s,
        ...
        max_known_csc_bytes,
        max_jacobian_evaluations: CAMPAIGN_JACOBIAN_EVALUATIONS,
        max_jacobian_entry_work,
        max_result_values,
...
fn solve(system: &DaeResidualSystem<'_>, settings: &IdaKluSettings) -> IdaSolveResult {
    IdaKluBackend::new()
        .expect("pinned native IDA/KLU backend")
        .initialize_session(system, settings)
        .expect("registered sparse IDA/KLU session")
...
fn assert_sparse_identity_and_success(result: &IdaSolveResult) {
    ...
    assert_ne!(identity.backend_id, NATIVE_IDA_BACKEND_ID);
    assert_ne!(identity.contract, NATIVE_IDA_BACKEND_CONTRACT);
    assert_eq!(identity.provider, "SUNDIALS + SuiteSparse");
    ...
    for expected in [
        ...
        assert!(identity.version.contains(expected));
    }
    assert_eq!(result.result_contract(), NATIVE_IDA_KLU_RESULT_CONTRACT);
    assert_ne!(result.result_contract(), NATIVE_IDA_RESULT_CONTRACT);
...
fn assert_closed_accounting(result: &IdaSolveResult) {
    assert_sparse_identity_and_success(result);
...
#[test]
fn klu_default_reject_preserves_fail_closed_scheduled_event_behavior() {
    let graph = step_events_graph(&[0.5]);
    let system = DaeResidualSystem::lower(&graph, 0.0, &core_settings()).unwrap();
    let settings = klu_settings(&system, 0.0, vec![0.75], IdaEventPolicy::Reject);
    let backend = IdaKluBackend::new().unwrap();
    assert_eq!(backend.identity().backend_id, NATIVE_IDA_KLU_BACKEND_ID);
    assert_eq!(backend.identity().linear_solver, "SUNLINSOL_KLU_COLAMD");
    assert!(backend.identity().sparse);
...
#[test]
fn klu_restart_result_binds_v2_contracts_backend_and_requested_policy() {
    ...
    let settings = klu_settings(&system, 0.0, vec![0.5, 0.75], policy);
    assert_eq!(system.variables().len(), 2);
    assert_eq!(system.csc_pattern().nonzero_count(), 3);
    assert_eq!(settings.max_dimension, 2);
    assert_eq!(settings.max_nonzeros, 3);
    assert_eq!(settings.max_known_csc_bytes, 96);
    assert_eq!(settings.max_jacobian_entry_work, 30_000);
    assert_eq!(settings.max_result_values, 4);
...
#[test]
fn klu_active_distinct_event_count_governs_restart_admission() {
    ...
    let limited = klu_settings(
        &system,
        0.0,
        vec![1.0],
        IdaEventPolicy::Restart { max_restarts: 1 },
    );
    assert_eq!(limited.max_dimension, 4);
    assert_eq!(limited.max_nonzeros, 4);
    assert_eq!(limited.max_known_csc_bytes, 136);
    assert_eq!(limited.max_jacobian_entry_work, 40_000);
    assert_eq!(limited.max_result_values, 4);
    assert_eq!(
...
#[test]
fn klu_restart_maximum_above_backend_ceiling_is_rejected() {
    let graph = step_events_graph(&[0.5]);
    let system = DaeResidualSystem::lower(&graph, 0.0, &core_settings()).unwrap();
    let at_ceiling = klu_settings(
        &system,
        0.0,
        vec![0.75],
    ...
    let above_ceiling = klu_settings(
        &system,
        0.0,
        vec![0.75],
        IdaEventPolicy::Restart {
...
#[test]
fn klu_initial_time_mismatch_is_rejected_while_signed_zero_matches() {
    let fixture = event_integrator_graph(0.25);
    let system = DaeResidualSystem::lower(&fixture.graph, -0.0, &core_settings()).unwrap();
    let matching = klu_settings(
        &system,
        0.0,
        vec![0.5],
        IdaEventPolicy::Restart { max_restarts: 1 },
...
#[test]
fn klu_single_event_grid_matches_left_equality_and_right_analytic_values() {
...
#[test]
fn klu_simultaneous_step_sources_share_one_restart_and_right_correction() {
    ...
    assert_eq!(settings.max_dimension, 4);
    assert_eq!(settings.max_nonzeros, 7);
    assert_eq!(settings.max_known_csc_bytes, 208);
    assert_eq!(settings.max_jacobian_entry_work, 70_000);
    assert_eq!(settings.max_result_values, 8);
...
#[test]
fn klu_two_distinct_events_restart_in_order_and_match_piecewise_integral() {
...
#[test]
fn klu_event_at_final_horizon_is_inclusive_and_publishes_terminal_equality() {
    let fixture = event_integrator_graph(0.5);
    let system = DaeResidualSystem::lower(&fixture.graph, 0.0, &core_settings()).unwrap();
    ...
    assert_closed_accounting(&reference);
...
#[test]
fn klu_event_at_initial_time_is_exclusive_and_needs_no_restart() {
...
#[test]
fn klu_event_one_ulp_after_final_is_inactive_and_cannot_contaminate_output() {
    ...
    let inactive_settings = klu_settings(
        &inactive_system,
        0.0,
        vec![final_time_s],
        IdaEventPolicy::Restart { max_restarts: 0 },
    ...
    let mixed_settings = klu_settings(
        &mixed_system,
        0.0,
        vec![final_time_s],
        IdaEventPolicy::Restart { max_restarts: 1 },
...
#[test]
fn klu_nonzero_initial_time_restart_matches_piecewise_integral() {
...
#[test]
fn klu_signed_zero_event_and_output_share_numeric_equality() {
    let fixture = event_integrator_graph(-0.0);
...
#[test]
fn klu_corrected_initial_conditions_remain_consistent_across_restart() {
    let fixture = event_integrator_graph(0.5);
    let system = DaeResidualSystem::lower(&fixture.graph, 0.0, &core_settings()).unwrap();
...
#[test]
fn klu_event_segment_and_correction_targets_fail_preflight_when_unrepresentable() {
    ...
    let too_close_settings = klu_settings(
        &too_close_system,
        initial_time_s,
        vec![2.0],
        IdaEventPolicy::Restart { max_restarts: 1 },
    ...
    let adjacent_settings = klu_settings(
        &adjacent_system,
        0.0,
        vec![2.0],
        IdaEventPolicy::Restart { max_restarts: 2 },
    ...
    let final_settings = klu_settings(
        &final_system,
        0.0,
        vec![f64::MAX],
        IdaEventPolicy::Restart { max_restarts: 1 },
    ...
    let usable_settings = klu_settings(
        &usable_system,
        initial_time_s,
        vec![2.0],
        IdaEventPolicy::Restart { max_restarts: 1 },
...
#[test]
fn klu_output_row_and_endpoint_custody_counters_close_exactly() {
    let event_time_s = 0.5;
    let immediately_before = previous_positive_f64(event_time_s);
    let fixture = event_integrator_graph(event_time_s);
    let system = DaeResidualSystem::lower(&fixture.graph, 0.0, &core_settings()).unwrap();
...
#[test]
fn klu_global_step_budget_is_cumulative_across_event_segments() {
    let fixture = two_event_integrator_graph_at(0.3, 0.6);
    let system = DaeResidualSystem::lower(&fixture.graph, 0.0, &core_settings()).unwrap();
    let mut settings = klu_settings(
        &system,
...
#[test]
fn klu_repeated_restart_sessions_are_bitwise_deterministic_including_stats() {
    let fixture = two_event_integrator_graph_at(0.3, 0.6);
    let system = DaeResidualSystem::lower(&fixture.graph, 0.0, &core_settings()).unwrap();
    let settings = klu_settings(
        &system,
    ...
    assert_closed_accounting(&second);
