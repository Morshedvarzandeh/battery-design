// Bays — non-rectangular envelopes: templates, polygon packing, shaped
...
test('box bay agrees with the closed-form rectangular engine', () => {
  // The polygon scanner may find a few extra cells in staggered rows —
  // never dramatically fewer.
...
test('round bay: fewer cells than its bounding box, all inside the circle', () => {
...
test('L-shape: less than the full rectangle, nothing in the cut corner', () => {
...
test('stepped bay: layer counts follow the zone heights', () => {
...
test('drawn polygon (triangle): packs, everything inside, area math right', () => {
...
test('shaped layout: builds to capacity, refuses beyond, carries series groups', () => {
...
test('maxFill in shaped mode respects capacity and the voltage window', () => {
...
test('a wall between the sample points is not invisible to the packer', () => {
  // rectFits used to probe four corners and four edge midpoints; with a
  // 40 mm cell those sit at x = -20, 0, +20, so a 10 mm slot at x = 5..15
  // was invisible to every probe and the packer placed cells the wall runs
  // straight through. The test is exact now.
  ...
  ok(straddling.length === 0, `no cell straddles the slot (found ${straddling.length})`);
