// Regressions for the space-first multi-objective flow: cells packed into a
// fixed envelope after component space is reserved, scored on
// energy / cost / mass with Pareto flagging.
...
// At least one Pareto candidate, and the top scorer under any weights is on the front.
ok(results.some((r) => r.pareto), 'a Pareto-optimal candidate exists');

// Pareto logic: a candidate beaten on all three axes must be dominated.
{
...
// Weights steer the ranking: pure-cost weights rank by cost among priced cells.
{
...
// Component space costs cells: a 10mm bottom plate in a tight-Z envelope
// must never fit more cells, and for 70mm-tall 21700s in z=90 it kills a use
// of vertical space elsewhere — check global energy never increases.
{
...
// Bigger envelope never packs less energy.
{
...
console.log(fails === 0 ? 'MAX-FILL REGRESSIONS PASSED' : `${fails} FAILURES`);
