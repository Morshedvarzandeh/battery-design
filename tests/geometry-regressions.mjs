// Regression: lying-orientation layouts must never exceed 100% packing
...
for (const arrangement of ['grid', 'hex']) {
  for (const orientation of ['upright', 'lying']) {
    for (const nz of [1, 2]) {
      ...
      ok(L.packingEfficiency <= 0.9069 + 1e-6, `${arrangement}/${orientation}/nz${nz} eff ${(L.packingEfficiency*100).toFixed(0)}% <= 90.7%`);
      ...
      for (let i = 0; i < L.positions.length; i++) for (let j = i + 1; j < L.positions.length; j++) {
        ...
        if (orientation === 'lying' && Math.abs(a.y - b.y) > 1e-6) {
          ok(Math.abs(a.y - b.y) >= c.dims.h - 1e-6, `${arrangement}/${orientation} row separation ${(Math.abs(a.y-b.y)).toFixed(1)} >= h`);
        } else {
...
// optimizeSpace should no longer produce >90.7% efficiency candidates for cylinders
const cands = optimizeSpace(c, 12, 3, {}, null, 8);
for (const cd of cands) {
  const innerVol = (cd.outer.x - 4) * (cd.outer.y - 4) * (cd.outer.z - 4 - 8);
  ok(cd.volumeL > 0, 'vol > 0');
}
console.log(fails === 0 ? 'REGRESSION CHECKS PASSED' : `${fails} FAILURES`);
