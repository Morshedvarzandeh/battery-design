...
export const ADDONS = [
  ...
  },
  {
    ...
    what: 'The parameterised model: OCV + R0 + RC branches with Arrhenius dependence, reversible entropic heat, a per-module thermal network with an ε-NTU coolant stream, and calendar plus cycle aging. Every coefficient is exposed.',
    provides: ['per-module temperatures', 'electrical and thermal traces', 'aging schedule'],
    needs: ['the desktop runner', 'a pack and a current or power profile'],
    why: 'Runs on the desktop because a real study is thousands of sub-stepped seconds and returns more detail than the browser screening model.',
  },
  {
    id: 'calibration', name: 'Governed model calibration', tier: 'desktop', status: 'shipped',
    surfaces: ['cli', 'local-api'],
    module: 'calibration-dataset.js + calibration-import.js + sim2.js', concepts: ['simulation'],
    what: 'The source/staged runner CLI importer normalizes an exactly mapped delimited or columnar-JSON trace into a versioned, checksummed canonical calibration dataset. The CLI and authenticated local API fit canonical datasets to allowlisted existing model parameters with bounded and fully counted optimizer work.',
    provides: ['canonical calibration dataset identity', 'source and normalization provenance', 'fitted parameter artifact and calibration evidence'],
    needs: ['an exact source-column mapping or canonical dataset', 'a known catalog cell and explicit S/P/operating context', 'separate data for later independent validation'],
  ...
  {
    id: 'equations', name: 'Rust equation-graph solver', tier: 'browser', status: 'shipped',
    ...
    needs: ['approved blocks and parameters', 'the native or WebAssembly Rust host; high-order SUNDIALS/IDA adapters remain separate work'],
  ...
  {
    id: 'blocks', name: 'Guided block composition studio', tier: 'browser', status: 'shipped',
    module: 'cosim-graph.js + cosim-studio.js', concepts: ['simulation', 'cosim'],
    what: 'A calm visual canvas over the Rust equation graph: guided, manual and automatic-draft modes; exact typed ports; deterministic graph import/export; live trace playback; and reviewable assistant recommendations and repairs that a named human must approve.',
    provides: ['a visual typed model graph', 'isolated Road and segmented Grid templates', 'deterministic Rust/Wasm graph transport', 'plain-language diagnostics with minimal repair proposals', 'human approval history'],
    needs: ['approved blocks and parameters', 'a generated Rust/WebAssembly core for authoritative browser runs'],
    why: 'Customers start from the result they need rather than a wall of blocks, while specialists can still inspect every connection, parameter, solver decision and evidence identifier.',
  },
  ...
  },
  {
    ...
    what: 'Generate a reproducible FMI 2.0 source-FMU build kit containing the model description, complete C stepping source and build instructions. The release pipeline separately compiles and packages the downloadable Linux/Windows .fmu. Its FMI 2.0 interface targets import pipelines used by Twin Builder, Simulink, GT-SUITE and Dymola; proprietary host acceptance is recorded separately per product and version.',
    provides: ['an editable source-FMU build kit', 'a documented coupling interface', 'a separately CI-built compiled release FMU'],
    needs: ['a C toolchain only when compiling the local source kit; a matching supported platform for the release binary'],
    why: 'FMI provides one standards-based integration boundary. Open-source schema, ABI and lifecycle checks establish interoperability evidence, but do not substitute for an import-and-step result in a named proprietary host version.',
  },
  {
  ...
  },
  {
    ...
    what: 'Execute the versioned software model against independent numerical acceptance ranges and deterministic test vectors. Each run checks model/checksum/solver identity, units, limits and exact repeatability instead of merely replaying a graph.',
    provides: ['versioned SIL test plans', 'calculation pass/fail evidence', 'repeatability and identity checks', 'required convergence, boundary, conservation and fault-test checklist'],
    needs: ['an executable software-model adapter', 'independent expected values or accepted ranges', 'a fixed graph, model and solver version'],
    why: 'A plotted trace is evidence only after it is checked against an independent requirement or known answer. SIL makes that calculation contract executable and reproducible.',
  },
  {
    ...
    needs: ['a named real-time target', 'production-representative I/O', 'measured cycle-time and fault-response evidence'],
    why: 'Calling browser playback HIL would hide the two facts that matter: deterministic deadlines and physical I/O. The contract makes both explicit before equipment is connected.',
  },
  {
    ...
    needs: ['a supported real-time operating system and target', 'I/O drivers and calibration', 'worst-case execution-time qualification'],
    why: 'The contract and evidence evaluator are shipped, but hard real-time depends on measured worst-case execution time, drivers and supported hardware. It must not be claimed from a browser or workstation run.',
  },
  {
